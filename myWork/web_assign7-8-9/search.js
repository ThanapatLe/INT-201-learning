...
function search(text){ //function search ที่ทำการรับ parameter เป็น text โดยที่มีการเช็ค ถ้าหากสิ่งที่รับมาตรงกับสินค้าก็จะแสดงสินค้า 
    if(text.length != 0){
          // let divC = document.querySelector("#indexOfCard");
        let item = products.filter((p)=> p.productName.toLowerCase().includes(text.toLowerCase()));
          // for(let p of products){
          //   if(p != products.filter((pro)=> pro.productName.toLowerCase().includes(text.toLowerCase()))){
          //     divC.setAttribute("class","d-none");
          //   }
          // }     
        render(item);
    }
...
//   let selectId = document.getElementById("dataOfProduct");
//   let p = selectId.getElementsByTagName("");

//   for(pro = 0 ; pro < p.length ; pro++){
//      pIn = p[pro].getElementById("")[0];
//      txtValue = pIn.
//   }
...
//   if(text.length != 0){
//   let itemShow = products.forEach(pInStock => { 
//     if(pInStock.productName.toLowerCase().includes(text.toLowerCase())){
//       render(text);
//       if(){
